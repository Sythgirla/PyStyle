...
try:
    from core import StyleFactory
except:
    from pystyle.core import StyleFactory
import yaml


def launch_settings(parent=None):    
    settings_window=tk.Toplevel(parent) if parent else tk.Tk()
    settings_window.title("PyStyle Settings")
    settings_window.configure(
    width=1000, 
    height=600
    )
    settings_window.resizable(False, False)
    run_settings(settings_window)
    if parent:
...
def run_settings(root):
    ...
    global custom1Palette, custom2Palette, custom3Palette, configYAML
    global ich, ico
    ...
    def chooseColor(type : str, override : bool = False):
        ...
        if override == False: #Override = False? Assume button to change color is pressed - thus open color picker
            try:
                ...
                throwError("ColorTypeNotFound", "warning")
    ...
    #Definitions to pop up a message to verify the certain actions. This is to prevent accidental swaps.
    def popupPreset():
                ...
                case "SaveCustomPreset":
                    match popupSaveCustom():
                        ...
                        case 1: 
                            ...
                            for x in nextVar: #Simple way of pushing all variables into the custom palette, looping for all entries in the list.
    ...
    def on_closing():
        global primaryColor, secondaryColor, tertiaryColor, textColor, warningColor, dangerColor
        global custom1Palette, custom2Palette, custom3Palette
        yamlconfig = {
            'current colors': [
                primaryColor,
                secondaryColor,
